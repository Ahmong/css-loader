...
var reflocalPlugin = postcss.plugin("postcss-module-reflocal", function(options) {
  return function(root) {
  	root.walkRules(rule => {
  		var selectorProcessor = selectorParser(selectors => {
  			selectors.walkPseudos(pSelector => {
          if (pSelector.value === ':reflocal') {
            pSelector.walkClasses(cssClass => {
              var importIndex = imports["$" + cssClass.value];
              if (typeof importIndex === "number") {
                var newClass = selectorParser.className({value: "___CSS_LOADER_IMPORT___" + importIndex + "___"});
                cssClass.replaceWith(newClass);
              }
            });
            pSelector.each(selector => {
              selector.remove();
              pSelector.parent.insertBefore(pSelector, selector);
            });
            pSelector.remove()
          }
        })
      })
      rule.selector = selectorProcessor.process(rule.selector, {lossless: false}).result;
    })
  }
});
