...
module.exports = function(content, map) {
	...
	processCss(content, map, {
      ...
      } else if (/^[-!]|\.js(\?[^!]*)?$|\.vue(\?[^!]*)?$/.test(imp.url)) {
        return "exports.i(require(\"" + imp.url + "\"), undefined" + ");";
			} else {
		...
		function importItemMatcher(item) {
			...
			var importUrl ;
			if (/^[-!]|\.js(\?[^!]*)?$|\.vue(\?[^!]*)?$/.test(importItem.url)) {
        importUrl = importItem.url;
      } else {
        importUrl = importUrlPrefix + importItem.url;
      }
